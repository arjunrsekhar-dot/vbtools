...
export default async function DealsPage() {
  const deals = await Promise.all(
    tools
      .filter((tool) => tool.discount)
      .map(async (tool) => ({
        ...tool,
        logoUrl: tool.logoUrl || await resolveToolIcon(tool.websiteUrl, tool.slug, tool.logoUrl)
      }))
  );
  return (
    <>
      ...
      <section className="section"><div className="container"><div className="section-heading compact-heading"><div><span className="kicker">Live offers</span><h2>{deals.length} deals worth grabbing.</h2></div></div><div className="deals-page-grid">{deals.map((tool) => <article className="deal-page-card" key={tool.id}><div className="deal-page-top"><ToolLogo name={tool.name} logo={tool.logo} logoColor={tool.logoColor} logoUrl={tool.logoUrl} /><span>{tool.category}</span></div><h2>{tool.name}</h2><p>{tool.shortDescription}</p><div className="deal-highlight">{tool.discount}</div>{tool.couponCode && <div className="coupon-box"><code>{tool.couponCode}</code><Copy size={15} /></div>}<Link href={`/tools/${tool.slug}`}>View deal details <ArrowRight size={15} /></Link></article>)}</div></div></section>
    </>
  );
}
