...
export default async function ToolDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const rating = tool.rating ? tool.rating.toFixed(1) : "—";
      ...
      <section className="detail-header">
          ...
          <div className="detail-hero-grid">
            <div className="detail-main">
              <ToolLogo name={tool.name} logo={tool.logo} logoColor={tool.logoColor} logoUrl={tool.logoUrl} className="detail-logo" />
              <div>
                ...
                <div className="detail-meta"><span className="rating"><Star size={15} fill="currentColor" /> {rating} <small>({tool.ratingCount} ratings)</small></span><i /><Link href={`/tools?category=${encodeURIComponent(tool.category)}`}>{tool.category}</Link><i /><span>Updated {new Date(tool.updatedAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</span></div>
              </div>
            </div>
